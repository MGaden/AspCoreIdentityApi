...
namespace CoreIdentity.API.Identity.Controllers
{
    ...
    public class ManageController : Controller
    {
        ...
        public async Task<IActionResult> SendVerificationEmail([FromBody] ConfirmEmailRequestViewModel model)
        {
            var user = await _userManager.FindByIdAsync(User.FindFirst("uid")?.Value).ConfigureAwait(false);
            if (user == null)
                return BadRequest(new string[] { "Could not find user!" });
        ...
        public async Task<IActionResult> Logout()
        {
            ...
            await _refreshToken.RevokeToken(User.FindFirst("uid")?.Value).ConfigureAwait(false);
